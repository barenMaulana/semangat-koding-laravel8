<div>
    ...
    @forelse ($withdrawals as $key => $row)
        <div class="flex flex-col bg-white shadow-lg rounded-lg mx-4 md:mx-auto my-10 max-w-md md:max-w-2xl "><!--horizantil margin is just for display-->
            ...
            <div class="w-full">
                ...
                <p class="text-gray-700">Jumlah : <div class="px-2 py-10 text-4xl text-center text-white font-bold leading-none bg-blue-500 rounded">Rp.{{number_format($row->amount)}}</div> </p>
                <div class="mt-4 flex items-center justify-around">
                    <button wire:click="approve({{$row->id}},'success')" class="bg-green-400 hover:bg-green-300 text-white font-bold py-2 px-4 rounded my-3">Approve</button>
                    <button wire:click="approve({{$row->id}},'reject')" class="bg-red-400 hover:bg-green-300 text-white font-bold py-2 px-4 rounded my-3">Reject</button>
                </div>
            </div>
            </div>
        </div>
